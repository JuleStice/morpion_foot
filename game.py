...
class MorpionFootball:
    ...
    def create_widgets(self):
        ...
        self.pass_button.grid(row=self.grid_size + 6, columnspan=self.grid_size + 3)

        self.draw_button = tk.Button(self.root, text="Match nul", command=self.confirm_draw)
        self.draw_button.grid(row=self.grid_size + 8, columnspan=self.grid_size + 3)

        self.draw_button = tk.Button(self.root, text="Quitter la partie", command=self.on_closing)
        self.draw_button.grid(row=self.grid_size + 10, columnspan=self.grid_size + 3)        

        # Ajouter les widgets pour la recherche en bas
        self.search_entry = tk.Entry(self.root)
        self.search_entry.grid(row=self.grid_size + 13, column=0, columnspan=2)

        self.search_button = tk.Button(self.root, text="Rechercher", command=self.search_player)
        self.search_button.grid(row=self.grid_size + 13, column=2)

        self.search_result_label = tk.Label(self.root, text="")
        self.search_result_label.grid(row=self.grid_size + 14, column=0, columnspan=self.grid_size + 3)
    ...
    def check_validity(self, player, row, col):
        col = self.column_categories[col]
        row = self.row_categories[row]
        if player in self.players_db:
            player_info = self.players_db[player]
            if ((row in self.players_db[player]["competitions"] or row == self.players_db[player]["country"] or row in self.players_db[player]["clubs"] or
...
def main():
    root = tk.Tk()
    app = MorpionFootball(root)  # Assurez-vous que MorpionFootball est la classe principale de votre jeu
    root.mainloop()
